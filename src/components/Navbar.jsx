    ...
    <header className="bg-black text-white fixed top-0 right-0 left-0">
      <nav className="px-4 py-4 max-w-7xl mx-auto flex justify-between items-center">
        <a href="" className="text-xl font-bold text-white">
          Design <span className="text-orange-500">AD</span>
        </a>
        {/* navItems for lg device */}
        <ul className="md:flex gap-12 text-lg hidden ">
          {navItems.map(({ path, link }, ind) => (
        ...
        <div className="text-white lg:flex gap-4 items-center hidden">
          ...
          <button
            onClick={openModal}
            className="bg-orange-500 px-6 py-2 font-medium rounded hover:bg-white hover:text-orange-500 hover:font-bold transition-all duration-300 ease-in"
          >
            Log in
          </button>
        </div>
      ...
      <div>
        ...
        >
          {navItems.map(({ path, link }, ind) => (
            <li key={ind} className="text-black">
